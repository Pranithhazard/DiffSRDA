...
from src.yasuda.cfd_model.enkf.sr_enkf import assimilate_with_existing_data
from src.yasuda.cfd_model.interpolator.torch_interpolator import interpolate
from src.yasuda.dataset import DatasetVorticityAssimilation, DatasetMakingObsInside
from src.yasuda.utils import read_pickle
...
def read_batch_of_data(
    test_dataset: DatasetVorticityAssimilation, offset: int, max_datanum: int = None
):
    hr_omega, hr_obsrv, lr_assim, lr_frcst, lr_omega = [], [], [], [], []

    lst_model_params = []

    max_i = len(test_dataset) if max_datanum is None else max_datanum

    for i in tqdm(range(len(test_dataset)), total=max_i):
        if max_datanum is not None and i + 1 > max_datanum:
            break

        test_dir = test_dataset.data_dirs[i]

        _hr_omega, _hr_obsrv, _lr_assim, _lr_frcst = test_dataset._read_data(test_dir)
        i_sim = os.path.basename(test_dir).split("_")[-1]
        _lr_omega = torch.from_numpy(np.load(f"{test_dir}/lr_omegas_{i_sim}.npy"))
        _model_params = read_pickle(f"{test_dir}/model_params_{i_sim}.pickle")

        hr_omega.append(_hr_omega[:, offset:])
        hr_obsrv.append(_hr_obsrv[:, offset:])
        lr_assim.append(_lr_assim[:, offset:])
        lr_frcst.append(_lr_frcst[:, offset:])
        lr_omega.append(_lr_omega[:, offset:])
        lst_model_params.append(_model_params)

    hr_omega = torch.cat(hr_omega, dim=0)
    hr_obsrv = torch.cat(hr_obsrv, dim=0)
    lr_assim = torch.cat(lr_assim, dim=0)
    lr_frcst = torch.cat(lr_frcst, dim=0)
    lr_omega = torch.cat(lr_omega, dim=0)

    return hr_omega, hr_obsrv, lr_assim, lr_frcst, lr_omega, lst_model_params


def read_batch_of_data_without_obs(test_dataset: DatasetMakingObsInside, offset: int):
    hr_omega, lr_assim, lr_frcst, lr_omega = [], [], [], []
    lst_model_params = []

    for test_dir in tqdm(test_dataset.data_dirs, total=len(test_dataset)):
        _hr_omega, _lr_assim, _lr_frcst = test_dataset._read_data(test_dir)
    ...
    return hr_omega, lr_assim, lr_frcst, lr_omega, lst_model_params
