...
class ConvTransformerSrDaNetVer02(nn.Module):
    def __init__(
        self,
        *,
        in_channels: int,
        ...
        kernel_size: int = 3,
        bias: bool = False,
        input_sampling_interval: int = 2,
    ):
        super(ConvTransformerSrDaNetVer02, self).__init__()
    ...
    def forward(self, xs: torch.Tensor, obs: torch.Tensor) -> torch.Tensor:
        ...
        x = xs.permute(0, 2, 1, 3, 4)  # exchange channel and time dim
        x = F.interpolate(
            x,
        ...
        x = x.view(-1, self.in_channels, self.hr_y_size, self.hr_x_size)
