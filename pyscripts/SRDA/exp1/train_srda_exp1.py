...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to SRDA Exp1 config YAML.",
    )
    ...
    if is_main_process():
        log_file = logs_path / "train_srda_exp1.log"
        logger.addHandler(FileHandler(log_file))

    if is_main_process():
        logger.info("\n*********************************************************")
        logger.info(
            "SRDA Exp1 training start: "
    ...
    if is_main_process():
        logger.info("Making SRDA dataloaders for Exp1...")
    ...
    # Latent model (not used for Exp1, but keep the logic for future reuse).
    ...
    if is_main_process():
        logger.info("Starting diffusion SRDA training loop for Exp1...")
    ...
    if is_main_process():
        logger.info("\n*********************************************************")
        logger.info("SRDA Exp1 training finished.")
        logger.info("*********************************************************\n")
    cleanup_ddp()
